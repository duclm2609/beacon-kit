...
def run(plan, network_configuration = {}, node_settings = {}, eth_json_rpc_endpoints = [], additional_services = [], metrics_enabled_services = []):
    ...
    plan.print('>>>>>>>> Main: Genesis file: {}'.format(genesis_files))
    ...
    plan.print('>>>>>>>> Main: CL: env_vars: {}'.format(env_vars))
    ...
    plan.print('>>>>>>>> Main: modified genesis files with ENV VARS: {}'.format(genesis_files))
    ...
    plan.print(">>>>>>>>>>>> Main: add seed node client with config: {}".format(seed_node_configs))
    ...
    if full_node_configs != {}:
        plan.print(">>>>>>>>>>>> Main: add full node with config: {}".format(full_node_configs))
    ...
    plan.print(">>>>>>>>>>>> Main: add CL validator nodes with config: {}".format(validator_node_configs))
